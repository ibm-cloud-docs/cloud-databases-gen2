...
Automatic cross-region backup copy can be configured on a database instance. Only one target copy region can be configured. Any supported region can be selected as the target, provided it is different from the database instance region. Automatic backup cross region copy can also be disabled through the UI/API or CLI by removing the configured target region from the database instance configuration.
