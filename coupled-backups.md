...
keywords: backups, new deployment, source deployment, backup, back up, ondemand backup, on-demand backup, on-demand back up, instance, coupled backups

---

{{site.data.keyword.attribute-definition-list}}

# Managing coupled backups
{: #dashboard-backups}
...
- **Lifecycle**: Backups are tied to the database instance lifecycle and are deleted when the instance is deleted
- **Retention**: Backups are kept for 30 days
- **Encryption**: Backups are encrypted at rest with AES-256 encryption
- **Restoration**: Backups can only be restored within the same region where they were created
- **Types**: Both automatic (daily) and on-demand (manual) backups are available
...
- {{site.data.keyword.databases-for}} backups are not downloadable. If you need a local backup, use the appropriate software. For example, [pg_dump](https://www.postgresql.org/docs/9.6/backup-dump.html){: external} is an effective tool for managing PostgreSQL backups.

Deleting a backup is permanent and cannot be undone. Ensure you no longer need the backup data before deletion.
{: important}

## Viewing backups in the UI
{: #backup-ui}
{: ui}
...
If you plan to make major changes to your instance, like scaling or removing databases, tables, or collections, on-demand backups are useful. It can also be useful if you need to back up on a schedule. On-demand backups are kept for 30 days.
...
### Creating an on-demand backup in the UI
{: #ondemand-backup-ui}
{: ui}
