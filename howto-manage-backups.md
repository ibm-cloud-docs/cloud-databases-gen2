...
```sh
ibmcloud resource service-instance-create <INSTANCE_NAME> <SERVICE-ID> databases-for-postgresql-gen2-standard <REGION> -p  '{"dataservices":{"restore_backup_id":"BACKUP_CRN"}}'
```
{: .pre}

* Change the value of `instance_name` to the name that you want for your new instance.
* The `service-id` is the type of instance, such as _databases-for-postgresql_ or _databases-for-mongo_.
* The `region` is where you want the new instance to be located, which must be a Gen 2 region. 
* The `restore_backup_id` is the backup that you want to restore.

The previous command will restore a backup to a machine of the same configuration as your original deployment.

#### Optional parameters in the CLI
{: #restore-cli}
{: cli}

Optional parameters are available through the CLI. Use them if you need to customize resources, change the host size, or use a Key Protect key for BYOK encryption on the new instance. See the following example:

```sh
ibmcloud resource service-instance-create <INSTANCE_NAME> <SERVICE-ID> standard <REGION> -p
'{"backup_id":"BACKUP_ID","key_protect_key":"KEY_PROTECT_KEY_CRN", "members_disk_allocation_mb":"DESIRED_DISK_IN_MB"}'
```
{: .pre}

The `members_host_flavor` should be an appropriate-sized host (see [the list of available values](/docs/cloud-databases-gen2?topic=cloud-databases-gen2-isolated-compute&interface=ui).
{: note}
...
```sh
`ibmcloud resource service-instance-create <INSTANCE_NAME> databases-for-mongodb standard us-south -p '{"restore_backup_id":"<BACKUP_ID>", "version": "<VERSION>"}'
```

To see a list of versions available, run `ibmcloud cdb deployables`.

Gen 2 {{site.data.keyword.databases-for}} only support one version per database. Over time, new versions will be released, and when a new version becomes available, you can move to that version with a restore from a backup. 
{: .note}
